...
const WhatNeed: React.FC = () => {
  const images: SlideDataItem[] = [
    { src: "/media/individuals/find_focus2.png", title: "Find focus", audioPath: "/media/audio/audio1.mp3" },
    { src: "/media/individuals/Preparing_an_event.png", title: "Preparing for an event", audioPath: "/media/audio/audio2.mp3" },
    { src: "/media/individuals/Overcoming_anger.png", title: "Overcoming anger", audioPath: "/media/audio/audio3.mp3" },
    { src: "/media/individuals/Coping_sadness.png", title: "Coping sadness", audioPath: "/media/audio/audio4.mp3" },
    { src: "/media/individuals/Managing_anxiety.png", title: "Managing anxiety", audioPath: "/media/audio/audio5.mp3" },
    { src: "/media/individuals/Calming_mind.png", title: "Calming mind", audioPath: "/media/audio/audio6.mp3" },
  ];
  ...
  const startScrubbing = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (currentPlaying !== null && audioRef.current?.src && audioRef.current.duration) {
        setIsScrubbing(true);
        audioRef.current.pause();
  ...
  const handleMouseUp = useCallback(() => {
    if (isScrubbing && audioRef.current?.duration) {
      ...
      audioRef.current.play().catch((e) => console.error("Audio resume play failed:", e));
            ...
            {images.map((item, index) => (
              <SwiperSlide key={index} className="flex justify-center">
                <div className="w-full max-w-sm mx-auto flex flex-col items-center">
                    ...
                    >
                      <div
                        className={`h-full rounded-full absolute left-0 top-0 ${
                          currentPlaying === index ? "bg-red-600" : "bg-gray-400"
                        }`}
                        style={{
                          width: `${progress}%`,
                  ...
                  <h3 className="font-semibold text-4xl text-red-700 mt-4">{item.title}</h3>
                </div>
              </SwiperSlide>
            ))}
          </Swiper>
