...
const StudentsMindSkills = () => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const toggleModal = () => setIsModalOpen(!isModalOpen);

  const HERO = CONTENT_CONFIG.UNIVERSITIES_PAGE.WELL_ABOVE.HERO;
  const CARDS = CONTENT_CONFIG.UNIVERSITIES_PAGE.WELL_ABOVE.CARDS;

  return (
    <div className="students-mind-root">
      <div className="container-custom">
        {/* Heading Section */}
        <div className="students-mind-heading-wrapper">
          <h2
            className="students-mind-heading-primary"
            dangerouslySetInnerHTML={{ __html: HERO.HEADING.join("<br/>") }}
          ></h2>
          <h6
            className="students-mind-sub-heading"
            dangerouslySetInnerHTML={{ __html: HERO.SUB_HEADING.join("<br/>") }}
          ></h6>
        </div>

        {/* Cards Section */}
        <div className="students-mind-cards-wrapper">
          <div className="students-mind-cards-column">
            {CARDS.filter((_, i) => i % 2 === 0).map((card, idx) => (
              <div key={idx} className="students-mind-card">
                <div className="students-mind-card-img-wrapper">
                  <LazyImage
                    src={card.img}
                    alt={card.title}
                    className="students-mind-card-img"
                  />
                </div>
                <div className="students-mind-card-content">
                  <h4 className="students-mind-card-title">{card.title}</h4>
                  <h3 className="students-mind-card-desc">{card.desc}</h3>
                </div>
              </div>
            ))}
          </div>
...
export default StudentsMindSkills;
